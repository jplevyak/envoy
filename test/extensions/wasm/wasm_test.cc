...
TEST(WasmTest, Stats) {
  ...
  EXPECT_CALL(*context, scriptLog(spdlog::level::trace, Eq("get counter = 1")));
  EXPECT_CALL(*context, scriptLog(spdlog::level::debug, Eq("get counter = 2")));
  // recordMetric on a Counter is the same as increment.
  EXPECT_CALL(*context, scriptLog(spdlog::level::info, Eq("get counter = 5")));
  EXPECT_CALL(*context, scriptLog(spdlog::level::warn, Eq("get gauge = 2")));
  // Get is not supported on histograms.
  EXPECT_CALL(*context, scriptLog(spdlog::level::err, Eq("get histogram = 0")));
